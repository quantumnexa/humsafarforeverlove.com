...
export default async function PaymentLiveePage({
  ...
  // Optional overrides for success/failure paths and additional query
  const successPath = typeof sp?.successPath === "string" && sp.successPath ? String(sp.successPath) : "/payment/success"
  const failurePath = typeof sp?.failurePath === "string" && sp.failurePath ? String(sp.failurePath) : "/payment/failure"
  const checkoutPath = typeof sp?.checkoutPath === "string" && sp.checkoutPath ? String(sp.checkoutPath) : "/payment/checkout"
  const successQuery = typeof sp?.successQuery === "string" && sp.successQuery ? String(sp.successQuery) : ""
  const failureQuery = typeof sp?.failureQuery === "string" && sp.failureQuery ? String(sp.failureQuery) : ""
  const customerEmail = typeof sp?.customerEmail === "string" && sp.customerEmail ? String(sp.customerEmail) : "someone234@gmai.com"
  const customerMobile = typeof sp?.customerMobile === "string" && sp.customerMobile ? String(sp.customerMobile) : "03000000090"
  const customerName = typeof sp?.customerName === "string" && sp.customerName ? String(sp.customerName) : "Merchant Test"
    ...
    <div style={{ padding: 20 }}>
      ...
      {!token ? (
        ...
        >
          ...
          <input type="hidden" name="MERCHANT_NAME" value={customerName} />
          ...
          <input
            type="hidden"
            name="SUCCESS_URL"
            value={`${siteUrl}${successPath}${successQuery ? `?${successQuery}` : ""}`}
          />
          <input
            type="hidden"
            name="FAILURE_URL"
            value={`${siteUrl}${failurePath}${failureQuery ? `?${failureQuery}` : ""}`}
          />
          <input
            type="hidden"
            name="CHECKOUT_URL"
            value={`${siteUrl}${checkoutPath}`}
          />
          <input
            type="hidden"
