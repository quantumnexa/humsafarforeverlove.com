...
create table if not exists public.payments (
  ...
  constraint payments_package_type_check check (
    package_type = any (array['basic','standard','premium','custom','add_on'])
  )
);
  ...
  if new.payment_status = 'accepted' then
    ...
    select
      ...
      case lower(new.package_type)
        ...
        when 'add_on'   then coalesce((select us.subscription_status from public.user_subscriptions us where us.user_id = new.user_id limit 1), 'Basic Package')
        else 'Basic Package'
      end,
      now(), now()
    where not exists (
